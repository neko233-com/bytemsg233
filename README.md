...
The short version: JSON is the main authoring path; `.proto` is available for metadata interchange. Generated code should not feel like Protobuf.
...
bytemsg233 export game.bmsg.json -f md,html,bmsg,proto -o ./protocol
```
...
YAML is still supported for teams that prefer it, legacy `.bmsg` can be exported for future tooling experiments, and `.proto` can be imported/exported as a proto3 metadata subset. The main authoring path is `.bmsg.json`.
...
## Export Protocol Docs and Schemas
...
**ByteMsg233** 是面向游戏、客户端和 SDK 的 JSON 优先二进制协议工具链。使用 `.bmsg.json` 作为主要协议描述 DSL，可导入/导出实用的 `.proto` 元数据子集，生成具有原生风格的代码，运行时库可直接复制到项目中使用（无需依赖包管理器发布）。

简而言之：JSON 是主编写路径，`.proto` 可用于元数据互通，生成的代码不应像 Protobuf 那样生硬。
...
仍支持 YAML（适合偏好 YAML 的团队），旧版 `.bmsg` 可导出用于未来工具实验，`.proto` 可作为 proto3 元数据子集导入/导出。主要创作路径是 `.bmsg.json`。
